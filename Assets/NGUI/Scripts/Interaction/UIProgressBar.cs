...
public class UIProgressBar : UIWidgetContainer
{
	...
	public bool mBlockEventOnEnable = false;
	...
	{
		if (mStarted && current == null && !mBlockEventOnEnable && EventDelegate.IsValid(onChange))
		{
			current = this;
			EventDelegate.Execute(onChange);
			current = null;
	...
	public void Start ()
	{
		...
		if (Application.isPlaying)
		{
			...
			if (current == null && !mBlockEventOnEnable && EventDelegate.IsValid(onChange))
			{
				current = this;
				EventDelegate.Execute(onChange);
				current = null;
