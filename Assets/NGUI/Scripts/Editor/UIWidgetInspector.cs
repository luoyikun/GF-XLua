...
public class UIWidgetInspector : UIRectEditor
{
    ...
    static void DrawMaxSize(SerializedObject so, UIWidget w, bool isPrefab)
    {
        GUILayout.BeginHorizontal();
        {
            NGUIEditorTools.DrawProperty("FillSize", so, "mFillWidth", GUILayout.MinWidth(100f));

            NGUIEditorTools.DrawProperty("x", so, "mFillHeight", GUILayout.MinWidth(30f));
        }
        GUILayout.EndHorizontal();
    }
