...
static public class NGUIEditorTools
{
	...
	static public bool MakeTextureReadable (string path, bool force)
	{
        ...
        //暂停资源检查
        //ProjectS.AssetPostprocessor.AssetPostprocessor.Enabled = false;
        //ProjectS.AssetPostprocessor.AssetPostprocessor.ClearCompressionProperty(ti);
        ...
        //ProjectS.AssetPostprocessor.AssetPostprocessor.Enabled = true;
		return true;
	}
    ...
    int x, int y, int width, int height, int insetWidth, int insetHeight, int borderLeft, int borderBottom, int borderRight, int borderTop,
    int border2Left, int border2Bottom, int border2Right, int border2Top)
    {
        if (!tex) return;
        ...
        if (Selection.activeGameObject == null || Selection.gameObjects.Length == 1)
        {
            ...
            GUI.BeginGroup(outerRect);
            {
                ...
                if (borderLeft > 0)
                {
                    ...
                    NGUIEditorTools.DrawTiledTexture(new Rect(x0, 0f, 2f, outerRect.height), tex);
                }

                if (borderRight > 0)
                {
                    float x1 = (float)(width - borderRight) / width * outerRect.width - 1;
                    NGUIEditorTools.DrawTiledTexture(new Rect(x1, 0f, 2f, outerRect.height), tex);
                }

                if (borderBottom > 0)
                {
                    float y0 = (float)(height - borderBottom) / height * outerRect.height - 1;
                    NGUIEditorTools.DrawTiledTexture(new Rect(0f, y0, outerRect.width, 2f), tex);
                }

                if (borderTop > 0)
                {
                    float y1 = (float)borderTop / height * outerRect.height - 1;
                    NGUIEditorTools.DrawTiledTexture(new Rect(0f, y1, outerRect.width, 2f), tex);
                }
                if (border2Left > 0)
                {
                    float x0 = (float)border2Left / width * outerRect.width - 1;
                    NGUIEditorTools.DrawTiledTexture(new Rect(x0, 0f, 1f, outerRect.height), tex);
                }

                if (border2Right > 0)
                {
                    float x1 = (float)(width - border2Right) / width * outerRect.width - 1;
                    NGUIEditorTools.DrawTiledTexture(new Rect(x1, 0f, 1f, outerRect.height), tex);
                }

                if (border2Bottom > 0)
                {
                    float y0 = (float)(height - border2Bottom) / height * outerRect.height - 1;
                    NGUIEditorTools.DrawTiledTexture(new Rect(0f, y0, outerRect.width, 1f), tex);
                }

                if (border2Top > 0)
                {
                    float y1 = (float)border2Top / height * outerRect.height - 1;
    ...
    static public void DrawSprite(Texture2D tex, Rect drawRect, Color color, Rect textureRect,Rect textureRect2, Vector4 border,Vector4 border2)
    {
        NGUIEditorTools.DrawSprite(tex, drawRect, color, null,
            Mathf.RoundToInt(textureRect.x),
            Mathf.RoundToInt(tex.height - textureRect.y - textureRect.height),
            Mathf.RoundToInt(textureRect.width),
            Mathf.RoundToInt(textureRect.height),
            Mathf.RoundToInt(textureRect2.width),
            Mathf.RoundToInt(textureRect2.height),
            Mathf.RoundToInt(border.x),
            Mathf.RoundToInt(border.y),
            Mathf.RoundToInt(border.z),
            Mathf.RoundToInt(border.w),
            Mathf.RoundToInt(border2.x),
            Mathf.RoundToInt(border2.y),
            Mathf.RoundToInt(border2.z),
            Mathf.RoundToInt(border2.w)
            );
    }
    /// <summary>
    /// Draw a sprite preview.
