...
public class NGUITransformInspector : Editor
{
	...
	public override void OnInspectorGUI ()
	{
		...
		int num = 2; //查两次 避免其它编辑器下的性能问题
		Transform temp = target as Transform;
		while(num > 0)
		{
			num--;
			if (!rects)
			{
				...
				{
					temp = temp.parent;
					if (temp.GetComponent<UIRect>() != null)
					{
						rects = true;
					}
				}
	...
	void DrawPosition (bool isRect)
	{
		...
		if (isRect && !Application.isPlaying)
		{
            float oldPosX = (target as Transform).localPosition.x;
            float oldPosY = (target as Transform).localPosition.y;
            serializedObject.ApplyModifiedProperties();
            Transform t = target as Transform;
            float newPosX = t.localPosition.x;
            float newPosY = t.localPosition.y;
			if (newPosX > oldPosX)
			{
				newPosX = Mathf.Ceil(newPosX);
			}
			else
			{
				newPosX = Mathf.Floor(newPosX);
			}
            if (newPosY > oldPosY)
            {
				newPosY = Mathf.Ceil(newPosY);
            }
            else
            {
				newPosY = Mathf.Floor(newPosY);
            }
			t.localPosition = new Vector3(newPosX, newPosY, Mathf.Round(t.localPosition.z));
			serializedObject.ApplyModifiedProperties();
        }
