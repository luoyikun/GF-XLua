...
public class UIRoot : MonoBehaviour
{
    ...
    private int m_WindowSizeFrame = -1;
...
#if UNITY_EDITOR
        if (m_WindowSizeFrame != frame || !Application.isPlaying)
#else
        if (m_WindowSizeFrame != frame)
#endif

        {
            m_WindowSizeFrame = frame;
            Vector2 size = NGUITools.screenSize;
            m_WorldSize = size * GetPixelSizeAdjustment(Mathf.RoundToInt(size.y));
        }
        return m_WorldSize;
    }
