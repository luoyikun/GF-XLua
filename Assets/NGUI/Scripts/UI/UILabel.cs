...
using ColorStyle = NGUIText.ColorStyle;
...
public class UILabel : UIWidget
{
    ...
    public static Func<string, ColorStyle, string> ReplaceStrColor;
	public static Func<string, ColorStyle, string> ReplaceStrColorByCode;
	...
	[HideInInspector, SerializeField] Direction mDirection = Direction.Horizonal;
	...
	[HideInInspector, SerializeField] Overflow mOverflow = Overflow.ClampContent;
	...
	[HideInInspector, SerializeField] ColorStyle mColorStyle = ColorStyle.DefaultStyle;
	...
	[System.NonSerialized] bool mUseBoldOrItalic = false;
	...
	public string text
	{
		...
		set
		{
			...
			{
				mText = value;
				//【PR2】【UI改版】【功能开发】【字色转ID】字典表中改为使用字色ID PA202110230059-1
				if(mEncoding && mText.Contains("#[_") && ReplaceStrColorByCode != null) 
				{
					//新版
					mText = ReplaceStrColorByCode(mText, mColorStyle);
				}
                else if (mColorStyle != ColorStyle.DefaultStyle && ReplaceStrColor != null)
				{
					//兼容旧版
                    //mText = ReplaceStrColor(mText, mColorStyle);
					//【PR2版本】【基础功能优化（PR2）】【性能优化】文本颜色格式化的优化处理 PA202203105295 现在不需要处理这个转换
				}
				MarkAsChanged();
				ProcessAndRequest();
...
//			}
	...
	public bool useBoldOrItalic
	{
		get
		{
			return mUseBoldOrItalic;
		}
		set
		{
			mUseBoldOrItalic = value;
		}
	}
	...
	public static void OnFontChanged (Font font)
	{
        ++mFontChangedDepth;
//#if UNITY_EDITOR
//        Debug.Log("OnFontChanged:" + font + " Thread.CurrentThread.ManagedThreadId:" + System.Threading.Thread.CurrentThread.ManagedThreadId + " mFontChangedDepth:" + mFontChangedDepth + "UIPanel.RefreshAllNGUIPanelFlag:" + UIPanel.RefreshAllNGUIPanelFlag + " UIPanel.UpdateFrameEnd:" + UIPanel.UpdateFrameEnd + " Time.frameCount:" + Time.frameCount);
//#endif
        ...
        if (mFontChangedDepth == 1||!HashFont.Contains(font))
        {
            ...
            //if (NGUIText.OnFontChangeUILabel != null)
            //{
            //    Debug.LogError("NGUIText.OnFontChangeUILabel:" + label);
    ...
    private static System.Collections.IEnumerator LateUpdateGeometry(Font font)
    {
        yield return null;
        ...
        for (int i = 0; i < mList.size; ++i)
        {
            ...
            {
                Font fnt = lbl.trueTypeFont;
                if (fnt == font && lbl.panel != null)
                {
                    //lbl.MarkAsChanged();
                    lbl.UpdateGeometry(frame);  // by zlq TT12556 【文字破碎】【客户端】查找并处理字体贴图过大的问题
                }
            }
        }
    }
    public static int FontChangedDepth
    {
        get
        {
            return mFontChangedDepth;
        }
    }
	...
	public void ProcessText (bool legacyMode = false, bool full = true)
	{
		...
		{
			if(mFinalFontSize <= 30)
			{
				mDensity = NGUIText.Font_Pixel_Density;
			}
			else
			{
				mDensity = 1.0f;
		...
		if (mFinalFontSize > 0)
		{
			...
			for (int ps = mFinalFontSize; ps > 0; --ps)
			{
				...
				else if (mOverflow == Overflow.ResizeFreely)
				{
                    ...
                    if (!(leftAnchor.target && rightAnchor.target))
                    {
                    ...
                    if (!(topAnchor.target && bottomAnchor.target))
                    {
                    ...
                    if (changeFlag && onChange != null) onChange();
	...
	public int GetCharacterIndexAtPosition (Vector3 worldPos, bool precise, string inputText = "")
	{
		Vector2 localPos = cachedTransform.InverseTransformPoint(worldPos);
		return GetCharacterIndexAtPosition(localPos, precise, inputText);
	}
	...
	public int GetCharacterIndexAtPosition (Vector2 localPos, bool precise, string inputText = "")
	{
		if (isValid)
		{
    ...
    protected override UIGeometry CreateGeometry()
    {
        //var geometry = PSNGUITools.GetGeometry();
        //geometry.verts = PSNGUITools.GetV3List();
        //geometry.mRtpVerts = PSNGUITools.GetV3List();
        //geometry.uvs = PSNGUITools.GetV2List();
        //geometry.cols = PSNGUITools.GetColorList();
        //return geometry;
        return PSNGUITools.GetGeometry();
    }
    /// <summary>
    ...
    public void PrintOverlay (int start, int end, UIGeometry caret, UIGeometry highlight, Color caretColor, Color highlightColor)
	{
		...
		//string text = processedText;
		string text = this.text; //by liuzhen 多行输入文本的问题
		UpdateNGUIText();
	...
	public void UpdateNGUIText ()
	{
		...
		NGUIText.useBoldOrItalic = mUseBoldOrItalic;
		...
		else
		{
            if (mFinalFontSize <= 30)
            {
				NGUIText.pixelDensity = NGUIText.Font_Pixel_Density;
            }
            else
            {
