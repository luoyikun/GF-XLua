...
public class UIPanel : UIRect
{
	...
	static public List<UIPanel> list = new List<UIPanel>(128);
    ...
    public List<UIWidget> widgets
    {
        get
        {
            if (m_Widgets == null)
            {
                m_Widgets = PSNGUITools.GetUIWidgetsList();
            }
            return m_Widgets;
        }
    }
    private List<UIWidget> m_Widgets;
	/// <summary>
    ...
    public List<UIDrawCall> drawCalls
    {
        get
        {
            if (m_DrawCalls == null)
            {
                m_DrawCalls = PSNGUITools.GetUIDrawCallList();
            }
            return m_DrawCalls;
        }
    }
    private List<UIDrawCall> m_DrawCalls;
    protected override void OnDestroy()
    {
        if (m_DrawCalls != null)
        {
            PSNGUITools.ReleaseUIDrawCallList(m_DrawCalls);
            m_DrawCalls = null;
        }
        if (m_Widgets != null)
        {
            PSNGUITools.ReleaseUIWidgetList(m_Widgets);
            m_Widgets = null;
        }
        base.OnDestroy();
    }
	...
	[HideInInspector][SerializeField] Vector2 mClipSoftness = new Vector2(0f, 0f);
	...
	public static readonly System.Comparison<UIPanel> m_CompareFunc = CompareFunc;
	...
	public int clipCount
	{
		get
		{
			...
			while (p != null)
			{
				if (p.mClipping == UIDrawCall.Clipping.SoftClip || p.mClipping == UIDrawCall.Clipping.TextureMask || mClipping == UIDrawCall.Clipping.TextureMaskReverse) ++count;
				p = p.mParentPanel;
			}
			return count;
		}
	...
	public bool hasClipping { get { return mClipping == UIDrawCall.Clipping.SoftClip || mClipping == UIDrawCall.Clipping.TextureMask || mClipping == UIDrawCall.Clipping.TextureMaskReverse; } }
	...
	public bool IsVisible (UIWidget w)
	{
		...
		if (!w.fillGeometry) return true; //by liuzhen 输入文本超框后无光标问题
	...
	void UpdateSelf ()
    {
        ...
        else
        {
			...
			for (int i = 0; i < drawCalls.Count; )
			{
				...
				if (dc.isDirty && !FillDrawCall(dc, needsCulling) && !dc.isEmptyDrawCall)
				{
	...
	{
		mSortWidgets = false;
		widgets.Sort(UIWidget.m_PanelCompareFunc);
	}
	...
	void FillAllDrawCalls ()
    {
        ...
        for (int i = 0, imax = widgets.Count; i < imax; ++i)
		{
			...
			if (w.isVisible && (w.hasVertices || w.IsEmptyWidget()))
			{
				var mt = w.material;
				...
				//原特效占位dc的处理优化
				//保留dc数据列表，但是不会增加渲染
				//(更好的处理方式应该还是用多摄像机，因为递归获取所有render修改其rq还是不太好)
				//当前widget是占位widget或前一个dc是占位dc，则必须单独作为一个dc添加到列表
				if (mat != mt || tex != tx || sdr != sd || w.IsEmptyWidget() || (dc != null && dc.isEmptyDrawCall))
				{
					if (dc != null && (dc.verts.Count != 0 || dc.isEmptyDrawCall))
					{
						drawCalls.Add(dc);
				...
				if (mat != null || sdr != null || tex != null)
				{
					if (dc == null)
					{
						...
						if (safeToDraw)
						{
							...
							dc.isEmptyDrawCall = w.IsEmptyWidget();
						}
					}
					else
					{
	...
	public void Refresh ()
	{
		mRebuild = true;
		mUpdateFrame = -1;
        if (list.Count > 0) RefreshAllNGUIPanel();
	}
	...
	public bool ConstrainTargetToBounds (Transform target, ref Bounds targetBounds, bool immediate)
	{
		...
		if (offset.sqrMagnitude > 0f)
		{
			if (immediate)
			{
				...
				{
					transform.localPosition = new Vector3(transform.localPosition.x + offset.x, transform.localPosition.y + offset.y, transform.localPosition.z);
					clipOffset = new Vector2(clipOffset.x - offset.x, clipOffset.y-offset.y);
