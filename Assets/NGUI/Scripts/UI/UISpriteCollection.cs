...
public class UISpriteCollection : UIBasicSprite
{
    ...
    [System.NonSerialized] Dictionary<int, Sprite> mSprites = new Dictionary<int, Sprite>();
    ...
    {
        outer = new Rect(mSprite.x, mSprite.y, mSprite.width, mSprite.height);
        inner = new Rect(mSprite.x + mSprite.borderLeft, mSprite.y + mSprite.borderTop,
    ...
    private static List<Sprite> mSortList = new List<Sprite>();
    private static Stack<Sprite> mStack = new Stack<Sprite>();
    ...
    public override void OnFill(List<Vector3> verts, List<Vector2> uvs, List<Color> cols)
    {
        ...
        {
            mSortList.Clear();
            var e = mSprites.GetEnumerator();
            while (e.MoveNext())
            {
                if (e.Current.Value!=null)
                {
                    mSortList.Add(e.Current.Value);
                }
            }
            mSortList.Sort((a, b) => a.depth == b.depth ? 0 : a.depth>b.depth?1:-1);

            for (int k = 0; k < mSortList.Count; k++)
            {
                var ent = mSortList[k];
                if (!ent.enabled) continue;
                ...
                GetOuterAndInner(out Rect outer, out Rect inner, tex);
                mOuterUV = outer;
                mInnerUV = inner;
                mFlip = ent.flip;
    ...
    public void Add(int obj, string spriteName,int depth, Vector2 pos, float width, float height)
    {
        AddSprite(obj, spriteName, pos, width, height, new Color32(255, 255, 255, 255), new Vector2(0.5f, 0.5f),0,Type.Simple,Flip.Nothing,true,depth);
    }
    /// <summary>
    /// Add a new sprite entry to the collection.
    /// </summary>

    public void Add(int obj, string spriteName, Vector2 pos, float width, float height)
    {
    ...
    public void Add(int obj, string spriteName, Vector2 pos, float width, float height, Color32 color)
    {
    ...
    public void AddSprite(int id, string spriteName, Vector2 pos, float width, float height, Color32 color, Vector2 pivot,
        float rot = 0f, Type type = Type.Simple, Flip flip = Flip.Nothing, bool enabled = true,int depth = 0)
    {
        if (mAtlas == null)
        {
    ...
    {
        Sprite s;
        if (mSprites.TryGetValue(id,out s))
        {
            Release(s);
    ...
    public bool SetSprite(int id, Sprite sp)
    {
        mSprites[id] = sp;
        if (!mChanged) MarkAsChanged();
        return true;
    ...
    public bool SetActive(int id, bool visible)
    {
        Sprite sp;
    ...
    public bool SetPosition(int id, Vector2 pos, bool visible = true)
    {
        Sprite sp;
