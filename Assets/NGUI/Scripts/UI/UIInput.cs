...
public class UIInput : MonoBehaviour
{
	...
	public OnReturnKey onReturnKey = OnReturnKey.Submit;
	...
	public UIPanel mConstraintPanel = null;
	public Transform mMoveTarget = null;
	private List<Vector3> mCaretWorldCorners = new List<Vector3>(4);
	private Vector3[] panelWorldCorner;
	private Vector3[] labelWorldCorner;
	...
	public void Set (string value, bool notify = true)
	{
        ...
        if (value != null)
        {
            if (onReturnKey == OnReturnKey.Submit) value = value.Replace("\\n", "");
	...
	public void Start ()
	{
		...
		mStarted = true;
		if (mConstraintPanel != null)
        {
			if (mMoveTarget == null)
			{
				mConstraintPanel = null;
			}
			else if (mConstraintPanel.clipping != UIDrawCall.Clipping.SoftClip)
            {
				mConstraintPanel = null;
			}
        }
...
#if MOBILE
	private bool m_IsKeyboardActive = false;
#endif
	...
	protected virtual void Update ()
	{
		...
		if (mWaitForKeyboard)
		{
            if(!m_IsKeyboardActive && mKeyboard != null && mKeyboard.active)
            {
		...
		if (mSelectMe != -1 && mSelectMe != Time.frameCount)
		{
			...
			else
			{
				...
				UICamera.currentTouch.pos = Input.mousePosition;
	...
	protected int GetCharUnderMouse ()
	{
		...
		float dist;
		bool b = p.Raycast(ray, out dist);
		return b ? mDrawStart + label.GetCharacterIndexAtPosition(ray.GetPoint(dist), false, value) : 0;
	}
	...
	/// <summary>
	/// return caret world position
	/// </summary>
	/// <returns></returns>
	protected void CalculateCaretPosition()
	{
		if (isSelected && mCaret != null)
		{
			Transform parent = mCaret.transform.parent;
			for (int i = 0; i < mCaret.geometry.verts.Count; i++)
			{
				if (i >= mCaretWorldCorners.Count)
				{
					mCaretWorldCorners.Add(parent.TransformPoint(mCaret.geometry.verts[i]));
				}
				else
				{
					mCaretWorldCorners[i] = parent.TransformPoint(mCaret.geometry.verts[i]);
				}
			}
		}
	}
    ...
    {
		panelWorldCorner = mConstraintPanel.worldCorners;
		labelWorldCorner = label.worldCorners;
		CalculateCaretPosition();
	}

	private bool IsCaretVisible()
    {
		if (mConstraintPanel != null && mCaretWorldCorners.Count > 0)
        {
			for(int i = 0; i < mCaretWorldCorners.Count; i++)
			{
				if (!mConstraintPanel.IsVisible(mCaretWorldCorners[i]))
                {
					return false;
                }
			}
	...
	private void MoveTarget(ref Vector3 deltaPos)
    {
		if (mMoveTarget != null)
        {
			mMoveTarget.Translate(deltaPos, Space.World);
			deltaPos = Vector3.zero;
			UpdateWorldCorners();
        }
	}

	// 根据光标和label的对齐方式自动移动label
	protected void MoveLabel()
	{
		if (mConstraintPanel != null)
		{
			...
			{
				// 左对齐
				if (panelWorldCorner[0].x < labelWorldCorner[0].x || (label.width < mConstraintPanel.baseClipRegion.z && panelWorldCorner[0].x > labelWorldCorner[0].x))
				{
					deltaPos.x = panelWorldCorner[0].x - labelWorldCorner[0].x;
					MoveTarget(ref deltaPos);
				}
				if (label.width > mConstraintPanel.baseClipRegion.z)
				{// label比panel宽度宽
					if (labelWorldCorner[2].x < panelWorldCorner[2].x)
					{
						// label右侧在panel内，则右移保证label填满panel
						deltaPos.x = panelWorldCorner[2].x - labelWorldCorner[2].x;
						MoveTarget(ref deltaPos);
					}
				}
				if (isSelected && !IsCaretVisible())
				{
					// 以下代码不要写在else里，上面的判断可能有精度问题
					// 光标在panel外，则按光标移动
					if (mCaretWorldCorners[0].x < panelWorldCorner[0].x)
					{
						// 可以往左拖拽，label右移
						deltaPos.x = panelWorldCorner[0].x - mCaretWorldCorners[0].x;
						MoveTarget(ref deltaPos);
					}
					else if (mCaretWorldCorners[2].x > panelWorldCorner[2].x)
					{
						// 可以往右拖拽，label左移
						deltaPos.x = panelWorldCorner[2].x - mCaretWorldCorners[2].x;
						MoveTarget(ref deltaPos);
					}
					// 避免因为光标在最左侧导致label右移
					if (panelWorldCorner[0].x < labelWorldCorner[0].x)
					{
						deltaPos.x = panelWorldCorner[0].x - labelWorldCorner[0].x;
						MoveTarget(ref deltaPos);
					}
			...
			else if (label.pivot == UIWidget.Pivot.BottomRight || label.pivot == UIWidget.Pivot.Right || label.pivot == UIWidget.Pivot.TopRight)
			{
				// 右对齐
				if (panelWorldCorner[2].x < labelWorldCorner[2].x || (label.width < mConstraintPanel.baseClipRegion.z && panelWorldCorner[2].x > labelWorldCorner[2].x))
				{
					deltaPos.x = panelWorldCorner[2].x - labelWorldCorner[2].x;
					MoveTarget(ref deltaPos);
				}
				if (label.width > mConstraintPanel.baseClipRegion.z)
				{// label比panel宽度宽
					if (labelWorldCorner[0].x > panelWorldCorner[0].x)
					{
						// label左侧在panel内，则左移保证label填满panel
						deltaPos.x = panelWorldCorner[0].x - labelWorldCorner[0].x;
						MoveTarget(ref deltaPos);
					}
				}
				if (isSelected && !IsCaretVisible())
				{
					...
					// 避免因为光标在最右侧导致label左移
					if (panelWorldCorner[2].x < labelWorldCorner[2].x)
					{
						deltaPos.x = panelWorldCorner[2].x - labelWorldCorner[2].x;
						MoveTarget(ref deltaPos);
					}
			...
			if (label.pivot == UIWidget.Pivot.TopLeft || label.pivot == UIWidget.Pivot.Top || label.pivot == UIWidget.Pivot.TopRight)
			{
				// 上对齐
				if (panelWorldCorner[2].y > labelWorldCorner[2].y || (label.height < mConstraintPanel.baseClipRegion.w && panelWorldCorner[2].y < labelWorldCorner[2].y))
				{
					deltaPos.y = panelWorldCorner[2].y - labelWorldCorner[2].y;
					MoveTarget(ref deltaPos);
				}
				if (label.height > mConstraintPanel.baseClipRegion.w)
				{// label比panel高度高
					if (labelWorldCorner[0].y > panelWorldCorner[0].y)
					{
						// label下侧在panel内，则下移保证label填满panel
						deltaPos.y = panelWorldCorner[0].y - labelWorldCorner[0].y;
						MoveTarget(ref deltaPos);
					}
				}
				if (isSelected && !IsCaretVisible())
				{
					// 以下代码不要写在else里，上面的判断可能有精度问题
					// 光标在panel外，则按光标移动
					if (mCaretWorldCorners[0].y < panelWorldCorner[0].y)
					{
						// 可以往下拖拽，label上移
						deltaPos.y = panelWorldCorner[0].y - mCaretWorldCorners[0].y;
						MoveTarget(ref deltaPos);
					}
					else if (mCaretWorldCorners[2].y > panelWorldCorner[2].y)
					{
						// 可以往上拖拽，label下移
						deltaPos.y = panelWorldCorner[2].y - mCaretWorldCorners[2].y;
						MoveTarget(ref deltaPos);
					}
				}
			}
			else if (label.pivot == UIWidget.Pivot.BottomLeft || label.pivot == UIWidget.Pivot.Bottom || label.pivot == UIWidget.Pivot.BottomRight)
			{
				// 下对齐
				if (panelWorldCorner[0].y > labelWorldCorner[0].y || (label.height < mConstraintPanel.baseClipRegion.w && panelWorldCorner[0].y < labelWorldCorner[0].y))
				{
					deltaPos.y = panelWorldCorner[0].y - labelWorldCorner[0].y;
					MoveTarget(ref deltaPos);
				}
				if (label.height > mConstraintPanel.baseClipRegion.w)
				{// label比panel高度高
					if (labelWorldCorner[2].y < panelWorldCorner[2].y)
					{
						// label上侧在panel内，则上移保证label填满panel
						deltaPos.y = panelWorldCorner[2].y - labelWorldCorner[2].y;
						MoveTarget(ref deltaPos);
					}
				}
	...
	public void UpdateLabel ()
	{
		if (label != null)
		{
...
#endif
			{
				...
				if (mBlankTex == null)
				{
					mBlankTex = new Texture2D(2, 2, TextureFormat.ARGB32, false) { name = "UI Input Blank Texture" };
					for (int y = 0; y < 2; ++y)
						for (int x = 0; x < 2; ++x)
...
//#endif
	...
	protected char Validate (string text, int pos, char ch)
	{
        ...
        else if (validation == Validation.NoReturn)
        {
            if (ch == '/') return (char)0;
            if (ch == '\\') return (char)0;
            if (ch == '\n') return (char)0;
            if (ch == '\r') return (char)0;
            return ch;
        }
