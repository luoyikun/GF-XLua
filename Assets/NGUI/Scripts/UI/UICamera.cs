...
public class UICamera : MonoBehaviour
{
	...
	static public bool isDragStarted
    {
        get
        {
			...
			return currentTouch.dragStarted;
        }
    }
	...
	//给UITouch使用
	public static GameObject LastRayHitObject;
	...
	public static readonly BetterList<UICamera>.CompareFunc m_CompareFunc = CompareFunc;
	...
	static public void Raycast (MouseOrTouch touch)
	{
		...
		mLastPos = touch.pos;
		LastRayHitObject = mRayHitObject;
	}
	...
	static float mClickTime = 0f;

	static float mClickValidTime = 0.1f;

	static GameObject mLastClickObj = null;
	/// <summary>
	...
	static public void Notify (GameObject go, string funcName, object obj)
	{
		...
		if (funcName == "OnClick")
		{
			if (Time.unscaledTime - mClickTime <= mClickValidTime && go != mLastClickObj)
			{
				return;
			}
			else
    ...
    /// </summary>
    static public OnScreenResizeBefore onScreenResizeBefore;
    public delegate void OnScreenResizeBefore();
    static public OnScreenResizeAfter onScreenResizeAfter;
    public delegate void OnScreenResizeAfter();
    ...
    void LateUpdate ()
	{
		...
		if (w != mWidth || h != mHeight)
		{
            ...
            if (onScreenResizeBefore != null)
                onScreenResizeBefore();
           ...
           if (onScreenResize != null)
            ...
            if (onScreenResizeAfter != null)
                onScreenResizeAfter();
        }
	}

    public static bool IsGuideMask;
	/// <summary>
	/// Process all events.
	/// </summary>
	...
	public void ProcessMouse ()
	{
		...
		for (int i = 0; i < 3; ++i)
		{
            ...
            {
                bool _isPressd = Input.GetMouseButton(i);
                if (_isPressd != m_MouseDown[i])
                {
                    pressed = _isPressd;
                    unpressed = !_isPressd;
                    m_MouseDown[i] = pressed;
                }
            }
	...
	private int InputTouchCount
	{
		get
		{
			return (GetInputTouchCount == null) ? Input.touchCount : GetInputTouchCount();
		}
	}
		...
		if (currentTouch.pressed != null)
		{
				...
				currentTouch.totalDelta.sqrMagnitude < drag))
			{
				...
				if (currentTouch.clickNotification != ClickNotification.None && currentTouch.pressed == currentTouch.current)
				{
					...
					if (Time.unscaledTime - mClickTime >= mClickValidTime || currentTouch.pressed == mLastClickObj )
					{
