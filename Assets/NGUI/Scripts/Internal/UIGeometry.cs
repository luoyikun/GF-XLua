...
public class UIGeometry
{
    ...
    public List<Vector3> mRtpVerts;// = new List<Vector3>();
	Vector3 mRtpNormal;
	Vector4 mRtpTan;
	...
	public bool hasVertices { get { return (verts != null && verts.Count > 0); } }
    ...
    public void Release()
    {
        if (verts!=null)
        {
            PSNGUITools.ReleaseV3List(verts);
            verts = null;
        }
        if (uvs != null)
        {
            PSNGUITools.ReleaseV2List(uvs);
            uvs = null;
        }
        if (cols != null)
        {
            PSNGUITools.ReleaseColorList(cols);
            cols = null;
        }
        if (mRtpVerts != null)
        {
            PSNGUITools.ReleaseV3List(mRtpVerts);
            mRtpVerts = null;
        }
        onCustomWrite = null;
    }
