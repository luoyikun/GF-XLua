...
static public class NGUIText
{
    ...
    [DoNotObfuscateNGUI] public enum ColorStyle
    {
		DefaultStyle,
		Style1,
		Style2,
	...
	static public float Font_Pixel_Density = 0.5f;
	...
	static public Direction direction = Direction.Horizonal;
	...
	static public bool useBoldOrItalic = false;
    ...
    /// Recalculate the 'final' values.
    /// </summary>
    static public UILabel UpdateOnFontChangeUILabel;
	static public void Update (bool request)
	{
		...
		{
			font.RequestCharactersInTexture("0)_-", finalSize, fontStyle);
            //if (Time.frameCount != indexFrame)
            //{
            //    indexFrame = Time.frameCount;
            //    UILabel.OnFontChanged(font);
            //}
...
#else
			//注意这里baseline取值从")"改为"0"，因为现在用的Title字体的")"位置有偏移。2022-1-6 16:34:24 by liuzhen
			//")"对应baseline29，而"0"对应34，(fontsize=50)
            if (!font.GetCharacterInfo('0', out mTempChar, finalSize, fontStyle) || mTempChar.maxY == 0f)
			{
				font.RequestCharactersInTexture("A", finalSize, fontStyle);
				{
                    //if (Time.frameCount != indexFrame)
	...
	static public void Align (List<Vector3> verts, int indexOffset, float printedWidth, int elements = 4)
	{
        ...
        {
            float padding = rectWidth - finalLineHeight + spacingY;
	...
	static public bool WrapText (string text, out string finalText, bool keepCharCount, bool wrapLineColors, bool useEllipsis = false)
	{
		...
		{
            float height = (maxLines > 0) ? Mathf.Min(regionWidth, finalLineHeight * maxLines) : regionWidth;
            maxLineCount = Mathf.FloorToInt(Mathf.Min(maxLineCount, height / finalLineHeight) + 0.01f);
        }
	...
	static public void Print (string text, List<Vector3> verts, List<Vector2> uvs, List<Color> cols, List<Vector3> sverts = null, List<Vector2> suvs = null, List<Color> scols = null)
	{
		...
		for (int i = 0; i < textLength; ++i)
		{
			...
			if (encoding && ParseSymbol(text, ref i, mColors, premultiply, ref subscriptMode, ref bold,
				ref italic, ref underline, ref strikethrough, ref ignoreColor, ref forceSpriteColor))
			{
				ignoreColor = true; //颜色始终不叠加 如果没有字色 取原底色 2021-6-18 16:06:35 by liuzhen
				if (ignoreColor)
				{
					if (mColors.size == 1)
					{
			...
			else // No symbol present
			{
				...
				else
				{
                    if (y + w > regionHeight)
                    {
                        ...
                        y = 0;
                        x -= finalLineHeight; //TODO: expand from left to right
                        prevX = 0;
                    }
                }
				...
				{
					y += w; //TODO: 很显然竖排文字的间距并非advance直接关联
                }
				...
				if (underline || strikethrough)
				{
                    ...
                    if (underline)
                    { // Project-S：下划线往下挪一点，厚度改为原来的0.7倍
                        float underlineThickness = Mathf.Abs(dash.v0.y - dash.v1.y);
                        float newThickness = 0.7f * underlineThickness;
                        float moveDown = Mathf.Abs(dash.v0.y / 4);

                        v0y = (-y + dash.v0.y - moveDown);
                        v1y = (-y + dash.v1.y - moveDown + (underlineThickness - newThickness));
                    }
                    else
                    {
					    v0y = (-y + dash.v0.y);
