...
public class UIWidget : UIRect
{
    ...
    [HideInInspector] [SerializeField] protected int mFillWidth = 0;
    [HideInInspector] [SerializeField] protected int mFillHeight = 0;
    ...
    private UIGeometry m_Geometry;
    protected virtual UIGeometry CreateGeometry()
    {
        return PSNGUITools.GetGeometry();
    }
    protected virtual void ReleaseGeometry(UIGeometry geo)
    {
        PSNGUITools.ReleaseGeometry(geo);
    }
    /// <summary>
    ...
    public static readonly System.Comparison<UIWidget> m_FullCompareFunc = FullCompareFunc;
    ...
    public static readonly System.Comparison<UIWidget> m_PanelCompareFunc = PanelCompareFunc;
    ...
    public void UpdateGeometryBuffer()
    {
        var vc = GetVertexCount();
        if (vc == -1 )
        {
            if (geometry.verts == null)
            {
                //CreateGeometryBuffer(geometry, vc);
            }
            else
            {
                ReleaseGeometryBuffer(geometry);
                geometry.verts = null;
                geometry.uvs = null;
                geometry.cols = null;
                geometry.mRtpVerts = null;
            }
        }
        else
        {
            ...
            {
                int min = geometry.verts.Capacity >> 1;
                
                if (vc > geometry.verts.Capacity||vc < min)
                {
                    ReleaseGeometryBuffer(geometry);
                    CreateGeometryBuffer(geometry, vc);
                }
    ...
    public void ReleaseGeometryBuffer(UIGeometry geo)
    {
        PSNGUITools.ReleaseColorList(geo.cols);
        PSNGUITools.ReleaseV2List(geo.uvs);
        PSNGUITools.ReleaseV3List(geo.verts);
        PSNGUITools.ReleaseV3List(geo.mRtpVerts);
    }
    public void CreateGeometryBuffer(UIGeometry geo,int cap)
    {
        geo.verts = PSNGUITools.GetV3List(cap);
        geo.mRtpVerts = PSNGUITools.GetV3List(cap);
        geo.uvs = PSNGUITools.GetV2List(cap);
        geo.cols = PSNGUITools.GetColorList(cap);
    }
    /// <summary>
    ...
    {
#if UNITY_EDITOR
        UnityEngine.Profiling.Profiler.BeginSample("UIWidgetStart In Editor Only");
    ...
    virtual public int GetVertexCount()
    {
        return -1;
    }

    virtual public bool IsEmptyWidget()
    {
        return false;
    }
