...
public abstract class UIBasicSprite : UIWidget
{
    ...
    [System.NonSerialized] protected Rect mDoubleInnerUV = new Rect();
	/// <summary>
	...
	public float fillAmount
	{
        ...
        set
        {
            ...
            //如果是sliced的类型 自动改size
            if (type == Type.Sliced)
            {
                if (mFillWidth > 0)
                {
                    if (width != mFillWidth * val)
                    {
                        mFillAmount = val;
                        mChanged = true;
                        width = (int)(mFillWidth * mFillAmount);
                    }
                }
                else
                {
                    if (height != mFillHeight * val)
                    {
                        mFillAmount = val;
                        mChanged = true;
                        height = (int)(mFillHeight * mFillAmount);
                    }
                }
            }
        }
    ...
    public override int GetVertexCount()
    {
        switch (type)
        {
            ...
            case Type.Tiled:
                {
                    GetOuterAndInner(out Rect outer, out Rect inner, mainTexture);
                    mOuterUV = outer;
                    mInnerUV = inner;
                    var v = drawingDimensions;
                    var idx = GetTiledFillVectexCount(ref v);
                    return idx;
                }
            case Type.Filled:
                return 32;
            case Type.Advanced:
                {
                    ...
                    mInnerUV = inner;
                    var v = drawingDimensions;
                    int idx = GetAdvancedFillVectexCount(ref v);
                    return idx;
                }
            case Type.DoubleSliced:
                return 128;
            default:
                return 32;
        }
    ...
    public abstract void GetOuterAndInner(out Rect outer, out Rect inner,Texture tex);
    /// <summary>
...
#region Fill Functions
    ...
    static protected Vector2[] mTempPos2 = new Vector2[4];
    static protected Vector2[] mTempUVs2 = new Vector2[4];
    /// <summary>
	...
	protected void Fill (List<Vector3> verts, List<Vector2> uvs, List<Color> cols, Rect outer, Rect inner)
	{
        ...
        switch (type)
		{
            ...
            case Type.DoubleSliced:
            ///DoubleSlicedFill(verts, uvs, cols, ref v, ref u, ref c);
            break;
    ...
    protected void DoubleSlicedFill(List<Vector3> verts, List<Vector2> uvs, List<Color> cols, ref Vector4 v, ref Vector4 u, ref Color gc)
    {
    ...
    protected int GetTiledFillVectexCount(ref Vector4 v)
    {
        int sum = 0;
        var tex = mainTexture;
        if (tex == null) return sum;

        var size = new Vector2(mInnerUV.width * tex.width, mInnerUV.height * tex.height);
        size *= pixelSize;
        if (size.x < 2f || size.y < 2f) return sum;
    ...
    protected int GetAdvancedFillVectexCount(ref Vector4 v)
    {
        int sum = 0;
        var tex = mainTexture;
	...
	/// </summary>
     ...
     protected	static void Fill (List<Vector3> verts, List<Vector2> uvs, List<Color> cols,
