...
public class UIDrawCall : MonoBehaviour
{
	...
	[DoNotObfuscateNGUI] public enum Clipping : int
	{
        ...
        TextureMaskReverse = 5,   	// Revert clipped using a texture rather than math
    }
	...
	public bool isEmptyDrawCall { get; set; }
	...
	void CreateMaterial ()
    {
		...
		if (panel != null && (panel.clipping == Clipping.TextureMask || panel.clipping == Clipping.TextureMaskReverse))
        {
            mTextureClip = true;
            shader = PSNGUITools.GetShader("Hidden/" + shaderName + textureClip);
        }
        else if (mClipCount != 0)
        {
            shader = PSNGUITools.GetShader("Hidden/" + shaderName + " " + mClipCount);
            if (shader == null) shader = PSNGUITools.GetShader(shaderName + " " + mClipCount);

            // Legacy functionality
            if (shader == null && mClipCount == 1)
            {
                mLegacyShader = true;
                shader = PSNGUITools.GetShader(shaderName + soft);
            }
        }
        else shader = PSNGUITools.GetShader(shaderName);

        // Always fallback to the default shader
        if (shader == null) shader = PSNGUITools.GetShader("Unlit/Transparent Colored");
        ...
        if (panel != null)
        {
            if (panel.clipping == Clipping.TextureMaskReverse)
            {
                mDynamicMat.SetFloat("_Reverse", 1f);
            }
            else if(panel.clipping == Clipping.TextureMask)
            {
                mDynamicMat.SetFloat("_Reverse", 0f);
            }
        }
    }
	...
	void UpdateMaterials ()
	{
		...
		if (mRebuildMat || mDynamicMat == null || mClipCount != panel.clipCount || mTextureClip != (panel.clipping == Clipping.TextureMask || panel.clipping == Clipping.TextureMaskReverse))
		{
			RebuildMaterial();
			mRebuildMat = false;
		}
...
//#endif
    ...
    private void  GenerateCachedIndexBuffer (int vertexCount, int indexCount)
	{
	...
	void OnWillRenderObject ()
	{
		...
		if (mTextureClip)
		{
			...
			UIPanel currentPanel = panel.parentPanel;
			var rootScale = panel.cachedTransform.localScale;
			if (currentPanel != null && currentPanel.hasClipping && currentPanel.clipping == Clipping.SoftClip)
			{
			...
			else
			{
				cr.x = cr.y = 0;
				cr.z = cr.w = 10000;
				SetClipping(1, cr, currentPanel.clipSoftness, 0f);
			}
		}
	...
	public static int[] ClipRange = null;
	public static int[] ClipArgs = null;
